...
## GOBI 개발자 Mika 기술 검토 (2026-06-05)

2026-06-04 GOBI Team 미팅에서 이 구상을 공유한 후, 다음 날 Mika로부터 기술적 피드백을 받았다. 핵심 요약:

- **Phase 1 (QnA)**: 현재 GOBI 기능으로 구현 가능. 프롬프트와 데이터 정제가 관건.
- **Phase 3 (코디네이터)**: Google Meet → Gemini 미팅 노트 생성 → **웹훅 트리거** → 에이전트 활성화 구조. Gemini가 녹취를 담당하므로 별도 녹취 에이전트 불필요. 어드민 전용 대화 공간 별도 필요.
- **Phase 2 (유저 연결)**: Cron 트리거 방식으로 구현 가능.
- **구조적 분류**: 기능 개발을 ① 툴/권한 (Google Workspace 등) ② 트리거 종류 (웹훅 / Cron / 외부) 두 축으로 정리.

→ **상세 내용**: [2026-06-05 Builders Lounge AI 코디네이터 GOBI Mika 피드백 Slack](<../slack/2026-06-05 Builders Lounge AI 코디네이터 GOBI Mika 피드백 Slack.md>)
