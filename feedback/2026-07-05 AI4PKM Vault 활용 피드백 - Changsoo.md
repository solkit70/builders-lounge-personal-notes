...
links:
  - "[[Topics/AI4PKM]]"
  - "[[AI/Initiatives/Builders Lounge/ideas/2026-07-04 Kim Jin-young - Beyond LLM Wiki]]"
external:
...
Changsoo_Vault는 2026년 2월 13일 원본 `jykim/ai4pkm-vault`를 기반으로 설치되어 약 5개월간 운영되고 있다. 원본이 제공한 파이프라인(Ingest → Roundup → Topics)과 스킬 체계는 그대로 유지하면서, 그 위에 라이브 방송 체계, VibeLearn AI 학습 시스템, 영상 제작, 커뮤니티 운영이라는 4개의 독자 영역을 얹은 형태로 진화했다. 원본 프롬프트 18개 중 6개를 수정하고 2개를 신설했으며, 스킬은 원본 25개에 13개 이상을 추가했다. 콘텐츠는 마크다운 기준 약 2,264개 파일로 성장했고, 월별 생산량은 설치 이후 꾸준히 증가해 6월에 최고치(679개)를 기록했다. 특징적인 점은 볼트 본체는 git 없이 운영하면서, 목적별로 4개의 독립 GitHub 레포(위성 레포)를 볼트 안팎에 배치한 "허브-위성" 구조를 만들었다는 것이다.
...
| 2026-07 (5일간) | 36 | 1 | Live #17 준비, 커피챗, 주간 뉴스레터 신설 |
...
| 파일 | 상태 | 변화 내용 |
| --- | --- | --- |
| CLAUDE.md | 전면 재작성 (73→115줄) | 원본을 AGENTS.md 참조 구조로 바꾸고, TodoWrite 규칙·워크플로우 커밋 포맷·Canvas 저장 경로·라이브 방송/Rundown/Remotion/VibeLearn 스킬 게이트 등 자체 운영 규칙으로 교체 |
| AGENTS.md | 수정 (189→197줄) | VibeLearn AI 학습 게이트, BeYouLifeUpWithUs 워크플로우, Limitless 링크 포맷 등 추가 |
| GEMINI.md | 수정 (26→60줄) | 멀티 에이전트(Claude/Gemini/Codex) 운영 반영 |
...
| 구분 | 항목 |
| --- | --- |
| 그대로 사용 (10) | ARP, DDO, DIR, DRB, EDM, GWR, ICB, IWA, TIU, Prompts/README |
| 수정해서 사용 (6) | ACB, CBH, EIC, GDR, PBU, RVA |
...
원본 스킬 중 23개를 유지하고 2개(create-gif-slides, road-trip-planning)를 제거했으며, **13개 이상을 직접 추가**했다. 추가된 스킬이 곧 이 볼트의 독자 영역이다.

| 영역 | 직접 추가한 스킬 |
| --- | --- |
| 라이브 방송 | live-broadcast, rundown-writer |
...
특히 `bedtime-story`, `hello-skill` 같은 연습용 스킬이 남아 있는 것은 스킬 제작 자체를 학습 주제로 다뤘다는 흔적이고("Skill로 나의 반복 업무 자동화 하기" Live 실험과 연결), live-broadcast → rundown-writer로 이어지는 스킬 계열은 반복 업무에서 규칙을 발견하면 스킬로 굳히는 패턴이 자리잡았음을 보여준다.
...
**새로 지은 층 (독자 확장)**: 라이브 방송 체계(Rundown 문서 규격 + 전용 스킬 2종), VibeLearn AI 학습 시스템(볼트 밖에서 시작해 볼트 안으로 통합), Remotion 영상 제작 파이프라인, Builders Lounge 커뮤니티 운영, 위성 레포 구조 자체. 이 층이 현재 볼트 활동량의 대부분을 차지하며, 원본에는 없는 이 볼트만의 정체성이다.
...
이 문서는 원저자 김진영님께 "내가 ai4pkm-vault를 얼마나 잘 활용하고 있는지, 어떤 점을 놓치고 있는지" 조언을 구하기 위한 자료다. 김진영님은 이 시스템의 설계자이자 LLMWiki 기반 지식관리 실전 연구자(Beyond LLM Wiki, CMDS 2026-07-02 발표)이므로, 아래 질문들은 설계 의도와 실제 사용의 간극을 확인하는 데 초점을 맞췄다. → **관련 발표**: [[AI/Initiatives/Builders Lounge/ideas/2026-07-04 Kim Jin-young - Beyond LLM Wiki]]
